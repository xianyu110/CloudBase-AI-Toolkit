...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=CreateUser)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：CreateUser。 |
| Version | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：2018-06-08。 |
| Region | 否 | String | [公共参数](/document/api/876/34812) ，此参数为可选参数。 |
| EnvId | 是 | String | 环境id  
示例值：test-envId |
| Name | 是 | String | 用户名，用户名规则：1. 长度1-64字符 2. 只能包含大小写英文字母、数字和符号. \_ - 3. 只能以字母或数字开头 4. 不能重复  
示例值：zhangsan |
| Uid | 否 | String | 用户ID，最多64字符，如不传则系统自动生成  
示例值：1001 |
| Type | 否 | String | 用户类型：internalUser-内部用户、externalUser-外部用户，默认internalUser（内部用户）  
示例值：internalUser |
| Password | 否 | String | 密码，传入Uid时密码可不传。密码规则：1. 长度8-32字符（推荐12位以上） 2. 不能以特殊字符开头 3. 至少包含以下四项中的三项：小写字母a-z、大写字母A-Z、数字0-9、特殊字符()!@#$%^&\*|?><\_-  
示例值：Abc@123456 |
| UserStatus | 否 | String | 用户状态：ACTIVE（激活）、BLOCKED（冻结），默认激活  
示例值：ACTIVE |
| NickName | 否 | String | 用户昵称，长度2-64字符  
示例值：张三 |
| Phone | 否 | String | 手机号，不能重复  
示例值：13800138000 |
| Email | 否 | String | 邮箱地址，不能重复  
示例值：zhangsan@example.com |
| AvatarUrl | 否 | String | 头像链接，可访问的公网URL  
示例值：https://example.com/avatars/zhangsan.jpg |
| Description | 否 | String | 用户描述，最多200字符  
示例值：用户描述 |
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| Data | [CreateUserResp](/document/api/876/34822#CreateUserResp) | 结果返回 |
