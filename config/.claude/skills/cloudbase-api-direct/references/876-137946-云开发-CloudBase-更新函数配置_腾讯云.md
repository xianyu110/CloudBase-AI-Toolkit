...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=UpdateFunctionConfiguration)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：UpdateFunctionConfiguration。 |
...
在线依赖安装，TRUE 表示安装，仅支持 Node.js 函数。
...
是否开启Dns缓存能力。只支持EVENT函数。

默认值：FALSE

  
示例值：FALSE |
| IgnoreSysLog | 否 | String | 

忽略系统日志上报

  
示例值：FALSE |
| EipConfig.N | 否 | Array of [FunctionEipConfigFixed](/document/api/876/34822#FunctionEipConfigFixed) | 
