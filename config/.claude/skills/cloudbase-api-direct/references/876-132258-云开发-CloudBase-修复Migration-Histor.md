...
本接口（RepairPGUserMigrationHistory）用于受控修复 history，只修改 user\_schema\_migrations，不执行 SQL。

默认接口请求频率限制：20次/秒。

推荐使用 API Explorer

[点击调试](https://console.cloud.tencent.com/api/explorer?Product=tcb&Version=2018-06-08&Action=RepairPGUserMigrationHistory)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/876/34812) ，本接口取值：RepairPGUserMigrationHistory。 |
...
### 示例1 RepairPGUserMigrationHistory
...
X-TC-Action: RepairPGUserMigrationHistory
<公共请求参数>
